...
void EmergencyCenter::activateSpecificSensor(int sensorID)
{
    std::vector<std::shared_ptr<Sensor>> sensorList = getSensorList();
...
void EmergencyCenter::deActivateAllByLocationPointer(std::shared_ptr<Location> locationPointer)
{
    deActivateAllInVector(getAllByLocationPointer(locationPointer));
}

void EmergencyCenter::deActivateSpecificSensor(int sensorID)
{
    std::vector<std::shared_ptr<Sensor>> sensorList = getSensorList();
    for( auto & s : sensorList ){
        if( s->getSensorID() == sensorID){
...
void EmergencyCenter::testAllByLocationPointer(std::shared_ptr<Location> locationPointer)
{
    testAllComponentsInVector(getAllByLocationPointer(locationPointer));
}

void EmergencyCenter::testSpecificSensor(int sensorID)
{
    std::vector<std::shared_ptr<Sensor>> sensorList = getSensorList();
    for( auto & s : sensorList ){
        if( s->getSensorID() == sensorID){
